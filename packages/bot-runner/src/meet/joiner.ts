...
export class MeetJoiner {
  ...
  async join(meetLink: string): Promise<JoinResult> {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        const cantJoinError = await this.page
  ...
  private async waitForAdmission(): Promise<JoinResult> {
    ...
    while (
      Date.now() - startTime <
      (this.options.joinTimeoutMs ?? MEETING_CONFIG.BOT_JOIN_TIMEOUT_MS)
    ) {
  ...
  private async checkForErrors(): Promise<JoinResult | null> {
    ...
    if (
      await this.page
        .locator(SELECTORS.meetingEnded)
        .isVisible({ timeout: 500 })
        .catch(() => false)
    ) {
      return {
    ...
    if (
      ...
      const bodyText = await this.page
        .evaluate(() => document.body?.innerText?.substring(0, 500) || 'No body')
      ...
      const hasCountdown = await this.page
        .locator('text=Returning to home screen')
        .isVisible({ timeout: 500 })
        .catch(() => false);
      const hasSecondsLeft = await this.page
        .locator('text=seconds left')
        .isVisible({ timeout: 500 })
        .catch(() => false);
