...
export async function botRoutes(server: FastifyInstance): Promise<void> {
     ...
     */
    server.get('/api/v1/bot/status/:sessionId', async (request: FastifyRequest, reply: FastifyReply) => {
     ...
     */
    server.post('/api/v1/bot/stop/:sessionId', async (request: FastifyRequest, reply: FastifyReply) => {
        const { sessionId } = request.params as { sessionId: string };
        const session = activeSessions.get(sessionId);
